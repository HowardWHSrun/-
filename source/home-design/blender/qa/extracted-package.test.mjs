...
const start=new URL('../../tmp/package-check/第6户_住宅与花园_P02_完整交付包/开始查看.html',import.meta.url).href;
const screenshot=async name=>page.screenshot({path:fileURLToPath(new URL('./'+'extracted-'+name+'.png',import.meta.url))});
...
finally{await fs.writeFile(new URL('./extracted-package-results.json',import.meta.url),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));await browser.close();}
